...
def local_partition(x, window_size: List[int]):
    B, H, W, C = x.shape
...
def local_reverse(windows, window_size: List[int], img_size: List[int]):
    H, W = img_size
    C = windows.shape[-1]
...
def global_partition(x, grid_size: List[int]):
    B, H, W, C = x.shape
...
@register_notrace_function  # reason: int argument is a Proxy
def global_reverse(windows, grid_size: List[int], img_size: List[int]):
    H, W = img_size
    C = windows.shape[-1]
...
class InterBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False,
        ...
        self.ffn = ffn
        self.norm1 = norm_layer(dim)
        self.attn = InterAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class WSFVitBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn_local = None if transformer_cfg.no_block_attn else PartitionAttentionCl(**attn_kwargs)

        self.layer = layer
        self.attn_inter = InterBlock(dim=self.dim,
        ...
        self.attn_global = PartitionAttentionCl(partition_type='grid', **attn_kwargs)
...
if classification:
    ...
    @register_model
    def wsfvit_p0_224(pretrained=False, **kwargs) -> WSFVit:
        return _create_wsfvit('wsfvit_p0_224', 'wsfvit_p0_224', pretrained=pretrained, **kwargs)
    ...
    @register_model
    def wsfvit_p1_256(pretrained=False, **kwargs) -> WSFVit:
        return _create_wsfvit('wsfvit_p1_256', 'wsfvit_p1_256', pretrained=pretrained, **kwargs)
